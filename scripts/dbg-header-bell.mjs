// Mide si la campana se desplaza al aparecer/ocultarse la hamburguesa.
import { spawn } from 'node:child_process';
import { mkdtempSync, rmSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
const profileDir = mkdtempSync(join(tmpdir(), 'hdr-'));
...
const bellX = () => evalJs(`Math.round(document.getElementById('btn-notificaciones').getBoundingClientRect().left)`);
const hamVisible = () => evalJs(`!document.getElementById('btn-configuracion').classList.contains('hidden')`);

console.log('hamburguesa oculta → campana x =', await bellX());
await evalJs(`document.getElementById('btn-perfil-sidebar').click()`);
await sleep(700);
console.log('hamburguesa visible → campana x =', await bellX(), '| hamburguesa:', await hamVisible());
await evalJs(`document.getElementById('btn-cavents-hub').click()`);
await sleep(700);
console.log('de vuelta (oculta) → campana x =', await bellX());
