// Mide el transform de la campana en estado fresco (hamburguesa oculta).
import { spawn } from 'node:child_process';
import { mkdtempSync, rmSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
const profileDir = mkdtempSync(join(tmpdir(), 'bell-'));
...
await sleep(800);
console.log('campana transform fresco (hamburguesa oculta):', await evalJs(`getComputedStyle(document.getElementById('btn-notificaciones')).transform`));
console.log('hamburguesa hidden:', await evalJs(`document.getElementById('btn-configuracion').classList.contains('hidden')`));
