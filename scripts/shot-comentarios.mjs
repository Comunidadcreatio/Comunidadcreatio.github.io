// Captura screenshot del cajón de comentarios abierto para inspección visual.
import { spawn } from 'node:child_process';
import { mkdtempSync, rmSync, writeFileSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
const profileDir = mkdtempSync(join(tmpdir(), 'shotcom-'));
const chrome = spawn('C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe', [
  '--headless=new', '--disable-gpu', '--no-sandbox', '--remote-debugging-port=9286',
  `--user-data-dir=${profileDir}`, '--window-size=420,900', 'about:blank'
], { stdio: 'ignore' });
const sleep = (ms) => new Promise(r => setTimeout(r, ms));
const getJson = async (u) => (await fetch(u)).json();
let v; for (let i = 0; i < 40; i++) { try { v = await getJson('http://127.0.0.1:9286/json/version'); break; } catch { await sleep(250); } }
const page = await (async () => { try { return await getJson('http://127.0.0.1:9286/json/new?about:blank'); } catch { return (await fetch('http://127.0.0.1:9286/json/new?about:blank', { method: 'PUT' })).json(); } })();
const ws = new WebSocket(page.webSocketDebuggerUrl);
await new Promise((res, rej) => { ws.onopen = res; ws.onerror = rej; });
let id = 0; const pend = new Map();
const logs = [];
ws.onmessage = (ev) => { const m = JSON.parse(ev.data); if (m.id && pend.has(m.id)) { pend.get(m.id)(m); pend.delete(m.id); return; } if (m.method === 'Runtime.exceptionThrown') logs.push('[EXC] ' + m.params.exceptionDetails?.text); };
  ...
  source: `(() => {
      ...
      window.fetch = async (input, init) => {
          ...
          if (u.includes('backend-fundacion-atpe.onrender.com')) {
              ...
              if (u.includes('/obras/1/comentarios')) return json({ comentarios: [{ id:1, autor:'A', texto:'hola', fecha:'2026-01-01' }] });
...
if (shot && shot.result) { writeFileSync('scripts/dbg-comentarios.png', Buffer.from(shot.result.data, 'base64')); console.log('guardado scripts/dbg-comentarios.png'); }
else console.log('sin screenshot', JSON.stringify(shot).slice(0,200));
