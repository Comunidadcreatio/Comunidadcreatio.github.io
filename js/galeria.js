...
import { abrirComentarios } from './comentarios.js?v=e4f0625add';
...
// vecina hacia la derecha para verse completa; a los ~2.5s vuelve a plegarse.
...
function iconoMeta(label) {
    ...
    return `<span class="obra-meta-ico" data-meta-label="${escapeHtml(label)}" role="button" tabindex="0" aria-label="${escapeHtml(label)}">${ICONO}</span>`;
}
...
function crearObraCard(obra) {
    ...
    const metaBar2HTML = (marcos || conservacion || firma) ? `
        <div class="obra-meta-bar obra-meta-bar-2">
            <span class="obra-meta-lado">
                ...
                ${conservacion ? `<span class="obra-meta-item">${iconoMeta('Conservación')}<span class="obra-meta-tooltip">Conservación</span><span class="obra-meta-texto">${escapeHtml(conservacion)}</span></span>` : ''}
            </span>
            <span class="obra-meta-lado obra-meta-lado-der">
    ...
    // El viewport muestra la imagen con su MISMO aspecto real (4:5 o 1:1 según
    // cómo la recortó el autor al subirla), tanto en el carrusel como en el
