...
} from './galeria-ui.js?v=16c6fb4927';
import {
...
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=02d2b805f9';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=16c6fb4927');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=5a7d7f1a7b').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=16c6fb4927').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
