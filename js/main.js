...
} from './galeria-ui.js?v=fc1b3651ff';
import {
...
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=3234299934';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=fc1b3651ff');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=e0d3ec2ac3').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=fc1b3651ff').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
