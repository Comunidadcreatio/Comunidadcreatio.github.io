...
} from './galeria-ui.js?v=4260504904';
import {
...
import { setupProblogs, abrirProblogDesdeNotificacion } from './problogs.js?v=3a90b9cacd';
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=8c26979a91';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    document.getElementById('notif-dropdown').classList.add('hidden');
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=4260504904');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=e246e85086').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=4260504904').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
