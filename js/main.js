...
import { token, artistaActual, logout, updateLastActivity } from './auth.js?v=30e2869c22';
...
} from './galeria-ui.js?v=861cf7b07d';
import {
    setupFormChangeTracking,
    setupImagePreviews, limpiarFormularioCompleto,
    setupObraFormSubmit, setupFormAccordions
} from './panel-ui.js?v=54a0002e59';
import { cargarGaleria, mostrarGaleria } from './galeria.js?v=cd9427a67d';
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=56f316616e';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=861cf7b07d');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=a8be6e7caf').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=861cf7b07d').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
    ...
    import('./cuenta.js?v=23e92af40b').then(m => m.setupMiCuenta());
...
async function init() {
    ...
    if (!sesionValida) {
        ...
        const { intentarReanudarSesionRecordada } = await import('./biometric-login.js?v=8630c4e062');
