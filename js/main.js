...
} from './galeria-ui.js?v=ef858e509a';
import {
    setupFormChangeTracking,
    setupImagePreviews, limpiarFormularioCompleto,
    setupObraFormSubmit, setupFormAccordions
} from './panel-ui.js?v=eb6c878bdb';
import { cargarGaleria, mostrarGaleria } from './galeria.js?v=1eef7a0c76';
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=d160c562ef';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=ef858e509a');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=e3de26799e').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=ef858e509a').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
