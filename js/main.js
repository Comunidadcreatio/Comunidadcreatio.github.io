...
} from './galeria-ui.js?v=af3cfaea7a';
import {
    setupFormChangeTracking,
    setupImagePreviews, limpiarFormularioCompleto,
    setupObraFormSubmit, setupFormAccordions
} from './panel-ui.js?v=54a0002e59';
import { cargarGaleria, mostrarGaleria } from './galeria.js?v=6c4690ba5b';
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=0b0177153c';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=af3cfaea7a');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=18ae7b274e').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=af3cfaea7a').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
