...
} from './galeria-ui.js?v=eee617c965';
import {
    setupFormChangeTracking,
    setupImagePreviews, limpiarFormularioCompleto,
    setupObraFormSubmit, setupFormAccordions
} from './panel-ui.js?v=54a0002e59';
import { cargarGaleria, mostrarGaleria } from './galeria.js?v=c940746301';
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=9b4e5e1c5a';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=eee617c965');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=771e5bcacf').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=eee617c965').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
