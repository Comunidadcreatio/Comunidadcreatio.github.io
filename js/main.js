...
} from './galeria-ui.js?v=bf44ac279a';
import {
...
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=4e31bc583d';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=bf44ac279a');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=f2058a627c').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=bf44ac279a').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
