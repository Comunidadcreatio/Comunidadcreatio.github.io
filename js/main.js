...
} from './galeria-ui.js?v=a7ea86c8e5';
import {
...
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=126a929e4b';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=a7ea86c8e5');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=5349cf4a0a').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=a7ea86c8e5').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
