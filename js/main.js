...
} from './galeria-ui.js?v=32e3d0efe1';
import {
...
import { setupProblogs, abrirProblogDesdeNotificacion } from './problogs.js?v=b7380454b2';
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=7dfbfad3e8';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    document.getElementById('notif-dropdown').classList.add('hidden');
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=32e3d0efe1');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=2249bba3c4').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=32e3d0efe1').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
