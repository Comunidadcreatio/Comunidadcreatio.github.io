...
} from './galeria-ui.js?v=b99fab6cb8';
import {
...
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=2ae40e1e40';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=b99fab6cb8');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=f160bf66e6').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=b99fab6cb8').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
