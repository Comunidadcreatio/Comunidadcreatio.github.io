...
} from './galeria-ui.js?v=edfd45a1b2';
import {
...
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=8c2e085235';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=edfd45a1b2');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=b09eca10b7').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=edfd45a1b2').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
