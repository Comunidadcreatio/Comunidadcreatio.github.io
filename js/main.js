...
} from './galeria-ui.js?v=2dfab4f535';
import {
    setupFormChangeTracking,
    setupImagePreviews, limpiarFormularioCompleto,
    setupObraFormSubmit, setupFormAccordions
} from './panel-ui.js?v=eb6c878bdb';
import { cargarGaleria, mostrarGaleria } from './galeria.js?v=075a248a88';
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=80b73df9e2';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=2dfab4f535');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=07a8ef6eb6').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=2dfab4f535').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
