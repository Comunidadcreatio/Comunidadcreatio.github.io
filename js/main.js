...
} from './galeria-ui.js?v=6416de1dc8';
import {
    setupFormChangeTracking,
    setupImagePreviews, limpiarFormularioCompleto,
    setupObraFormSubmit, setupFormAccordions
} from './panel-ui.js?v=eb6c878bdb';
import { cargarGaleria, mostrarGaleria } from './galeria.js?v=459b098ef8';
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=9b337114d7';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=6416de1dc8');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=95ae181869').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=6416de1dc8').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
