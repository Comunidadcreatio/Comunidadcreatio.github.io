...
} from './galeria-ui.js?v=b222d5de9e';
import {
    setupFormChangeTracking,
    setupImagePreviews, limpiarFormularioCompleto,
    setupObraFormSubmit, setupFormAccordions
} from './panel-ui.js?v=a1457f8853';
import { cargarGaleria, mostrarGaleria } from './galeria.js?v=5193b237c3';
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=a466a06923';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=b222d5de9e');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=a248362cfc').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=b222d5de9e').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
