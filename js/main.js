...
} from './galeria-ui.js?v=cd594e21a7';
import {
...
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=a900a038c3';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=cd594e21a7');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=a3f60ef7c8').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=cd594e21a7').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
