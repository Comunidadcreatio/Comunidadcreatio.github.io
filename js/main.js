...
} from './galeria-ui.js?v=155525638c';
import {
    setupFormChangeTracking,
    setupImagePreviews, limpiarFormularioCompleto,
    setupObraFormSubmit, setupFormAccordions
} from './panel-ui.js?v=eb6c878bdb';
import { cargarGaleria, mostrarGaleria } from './galeria.js?v=4d97fc5ab3';
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=d84883b37c';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=155525638c');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=3ca2d51591').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=155525638c').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
