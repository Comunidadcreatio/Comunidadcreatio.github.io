...
} from './galeria-ui.js?v=d8964cbf63';
import {
    setupFormChangeTracking,
    setupImagePreviews, limpiarFormularioCompleto,
    setupObraFormSubmit, setupFormAccordions
} from './panel-ui.js?v=cac0ce069f';
import { cargarGaleria, mostrarGaleria } from './galeria.js?v=b85678fee0';
import { setupProblogs, abrirProblogDesdeNotificacion } from './problogs.js?v=ce9d61bfd9';
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=6f66cffedd';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    document.getElementById('notif-dropdown').classList.add('hidden');
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=d8964cbf63');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=4689450e34').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=d8964cbf63').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
