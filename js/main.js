...
} from './galeria-ui.js?v=0710c2dda1';
import {
...
import { setupChat, refrescarChatNoLeidos } from './chat.js?v=ad55987165';
        ...
        list.querySelectorAll('.notif-item').forEach(item => {
            item.addEventListener('click', async () => {
                ...
                if (obraId) {
                    ...
                    // Mostrar galería
                    const galeriaUI = await import('./galeria-ui.js?v=0710c2dda1');
...
function setupEvents() {
    ...
    import('./busqueda.js?v=1e0184e81e').then(m => {
        m.setupBuscador(
        ...
        if (!galeria.classList.contains('hidden')) {
            ...
            cargarGaleria(gc).then(obras => {
                mostrarGaleria(obras, gc, null, (artistaId) => {
                    import('./galeria-ui.js?v=0710c2dda1').then(m => m.verPerfilArtistaDesdeGaleria(artistaId));
                });
                setTimeout(() => {
